...
class MovePreview:
    ...
    def format_summary(self) -> str:
        ...
        if self.affected_links:
            ...
            lines.extend(
                f"   • {link.file_path}:{link.line}" for link in self.affected_links[:5]
            )
        ...
        if self.redirects_needed:
            lines.append(
                f"↪️  {len(self.redirects_needed)} redirect(s) would be created:"
            )
            lines.extend(
...
class PageDraft:
    ...
    def format_preview(self, max_lines: int = 20) -> str:
        ...
        if len(self.content.split("\n")) > max_lines:
            lines.append(
                f"   │ ... ({len(self.content.split(chr(10))) - max_lines} more lines)"
            )
...
class ContentMigrator(DebugTool):
    ...
    def execute_move(
        ...
        actions.append(
            f"Moved {preview.operation.source} → {preview.operation.destination}"
        )
    ...
    def _find_structure_issues(self) -> list[DebugFinding]:
        ...
        for page in self.site.pages:
            ...
            if (
                url
                and incoming_links.get(url, 0) == 0
                and not self._is_in_navigation(page)
            ):
