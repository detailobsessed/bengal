        ...
        base.update(
            {
                ...
                "last_successful_build": (
                    self.last_successful_build.isoformat()
...
def create_dev_error(
    ...
    *,
    ...
    context = DevServerErrorContext(
        ...
        if isinstance(trigger_file, str)
        else trigger_file,
...
def _generate_quick_actions(
    error: Exception, context: DevServerErrorContext
) -> list[str]:
    """
...
def _check_auto_fixable(error: Exception, context: DevServerErrorContext) -> None:
    ...
    if "undefined" in error_msg and "variable" in error_msg:
        ...
        if match:
            var = match.group(1)
            context.auto_fix_command = f"# In template, change: {{{{ {var} }}}} to: {{{{ {var} | default('') }}}}"
