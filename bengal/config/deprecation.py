...
DEPRECATED_KEYS: dict[str, tuple[str, str, str]] = {
    ...
    "fingerprint_assets": (
        "assets",
        "fingerprint",
        "Use `assets.fingerprint: true` instead.",
    ),
    "generate_sitemap": (
        "features",
        "sitemap",
        "Use `features.sitemap: true` instead.",
    ),
    "generate_rss": ("features", "rss", "Use `features.rss: true` instead."),
    "markdown_engine": (
        "content",
        "markdown_parser",
        "Use `content.markdown_parser` instead.",
    ),
    "validate_links": (
        "health.linkcheck",
        "enabled",
...
def migrate_deprecated_keys(
    config: dict[str, Any], in_place: bool = False
) -> dict[str, Any]:
    """
...
def get_deprecation_summary() -> str:
    ...
    summary.append(
        "Please update your `bengal.toml` or `bengal.yaml` files accordingly."
    )
    summary.append("")
