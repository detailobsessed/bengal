...
def create_site_snapshot(site: Site) -> SiteSnapshot:
    ...
    return SiteSnapshot(
        ...
        data=MappingProxyType(
            dict(site.data) if hasattr(site, "data") and site.data else {}
        ),
        menus=menus,
    ...
    for path, old_page in old_pages_by_path.items():
        if path in affected_paths:
            ...
            if path in site_pages_by_path:
                ...
                new_page_cache[id(mutable_page)] = _snapshot_page_initial(
                    mutable_page, site
                )
...
def _resolve_navigation(page_cache: dict[int, PageSnapshot], site: Site) -> None:
    ...
    for idx, path in enumerate(sorted_paths):
        ...
        next_page = (
            pages_by_path.get(sorted_paths[idx + 1])
...
def _snapshot_menu_item(
    ...
    )  # Fallback to name if title not available
    item_is_active = getattr(
...
def _most_common_template(pages: tuple[PageSnapshot, ...]) -> str:
    ...
    return (
        max(template_counts.items(), key=lambda x: x[1])[0] if template_counts else ""
    )
                    ...
                    try:
                        ...
                        if not hasattr(loader, "get_source"):
                            raise AttributeError(
                                "Loader does not have get_source method"
                            )
                        get_source = cast(
                            Callable[
                                [Any, str],
                        ...
                        if not hasattr(env, "parse"):
                            raise AttributeError(
                                "Environment does not have parse method"
                            )
                        parse_method = cast(Callable[[str], Any], env.parse)
                        ast = parse_method(source)
...
class SpeculativeRenderer:
    ...
    def prediction_accuracy(self) -> float:
        ...
        accuracy = min(total_predicted, total_actual) / max(
