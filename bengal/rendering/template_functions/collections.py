...
def resolve_pages(page_paths: list[str], site: SiteLike) -> list[Any]:
    ...
    # Access get_page_path_map via getattr (not in SiteLike protocol)
    get_page_path_map_fn = getattr(site, "get_page_path_map", None)
    if not get_page_path_map_fn:
        return []
    page_map = get_page_path_map_fn()
...
def group_by_month(
    items: list[Any], date_attr: str = "date"
