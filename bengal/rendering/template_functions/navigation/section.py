...
def get_section(path: str, site: SiteLike) -> Section | None:
    ...
    # Access get_section_by_path via getattr (not in SiteLike protocol)
    get_section_fn = getattr(site, "get_section_by_path", None)
    if get_section_fn:
        return get_section_fn(normalized)
    return None
...
def section_pages(path: str, site: SiteLike, recursive: bool = False) -> list[Page]:
    ...
    if not section:
        return []
    return (
        list(section.get_all_pages(recursive=True))
        if recursive
        else list(section.pages)
    )
