...
def _build_section_menu_item(
    ...
    # Access _dev_menu_metadata via getattr (internal attribute not in SiteLike protocol)
    dev_menu_metadata = getattr(site, "_dev_menu_metadata", None)
    if dev_menu_metadata is not None and dev_menu_metadata.get("exclude_sections"):
        excluded_sections = dev_menu_metadata["exclude_sections"]
