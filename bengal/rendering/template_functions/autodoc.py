...
class MemberView:
    ...
    def from_doc_element(cls, el: DocElementLike) -> MemberView:
        ...
        if meta and hasattr(meta, "signature"):
            ...
            raw_params = get_function_parameters(
                cast("DocElement", el), exclude_self=True
            )
...
class CommandView:
    ...
    def from_doc_element(cls, el: DocElementLike) -> CommandView:
        ...
        arguments = [
            c for c in children if getattr(c, "element_type", "") == "argument"
        ]
        subcommands = [
            c
            for c in children
            if getattr(c, "element_type", "") in ("command", "command-group")
        ]
