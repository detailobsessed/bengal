...
def _generate_go_sample(
    ...
    if request_body and method in ("POST", "PUT", "PATCH"):
        ...
        lines.append(
            f'\treq, err := http.NewRequest("{method}", url, bytes.NewBuffer(body))'
        )
    else:
    ...
    lines.append("")
    lines.append(
        f'\treq.Header.Set("Authorization", "{auth_scheme} " + os.Getenv("API_KEY"))'
    )
...
def _dict_to_php_array(obj: Any, indent: int = 2) -> str:
    ...
    if isinstance(obj, dict):
        items = [
            f"'{k}' => {_dict_to_php_array(v, indent + 1)}" for k, v in obj.items()
        ]
        return "[" + ", ".join(items) + "]"
    return str(obj)
