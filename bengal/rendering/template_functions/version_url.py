...
def get_version_target_url(
    ...
    if section_index_url and page_exists_in_version(
        section_index_url, target_version_id, site
    ):
...
def _construct_version_url(
    ...
    sections = version_config.sections if version_config else ["docs"]

    # Determine current version prefix (empty for latest)
    current_prefix = ""
    if current_version_id and version_config:
        current_version = version_config.get_version(current_version_id)
