...
@handle_cli_errors()
def affected(
    file_path: str | None, by_hash: str | None, by_path: str | None, source: str
) -> None:
    ...
    if by_path:
        ...
        if not subvenance_path.exists():
            cli.warning(
                "No subvenance index found. Run 'bengal provenance build' first."
            )
            return
    ...
    if not affected_pages:
        ...
        cli.info(
            "  - No provenance has been captured yet (run 'bengal provenance build')"
        )
        cli.info(
            "  - The file was recently changed (hash doesn't match stored provenance)"
        )
