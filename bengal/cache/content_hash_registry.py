...
class ContentHashRegistry:
    ...
    def update_output(
        ...
        if hasattr(output_type, "name"):
            type_name = str(output_type.name)
        else:
    ...
    def load(cls, path: Path) -> ContentHashRegistry:
        ...
        zst_path = (
            path.with_suffix(".json.zst") if not path.name.endswith(".zst") else path
        )
    ...
    def validate(cls, path: Path) -> tuple[bool, str]:
        ...
        try:
            ...
            if version != REGISTRY_FORMAT_VERSION:
                return (
                    False,
                    f"Version mismatch: {version} != {REGISTRY_FORMAT_VERSION}",
                )
            ...
            return (
                True,
                f"Valid (version {version}, {len(data['source_hashes'])} sources)",
            )
