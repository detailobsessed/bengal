...
def _strip_bom(
    content: str, file_path: Path, encoding: str, caller: str | None = None
) -> str:
    """
...
def read_text_file(
    ...
    if not file_path.exists():
        logger.warning(
            "file_not_found", path=str(file_path), caller=caller or "file_io"
        )
        if on_error == "raise":
...
def write_json(
    ...
    try:
        ...
        write_text_file(
            file_path, content, create_parents=create_parents, caller=caller
        )
