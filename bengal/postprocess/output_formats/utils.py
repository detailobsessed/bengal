...
def get_page_relative_url(page: PageLike, site: Any) -> str:
    ...
    """
    # Use _path (internal path without baseurl) via getattr for protocol compatibility
    # Concrete Page class has _path, but PageLike protocol only exposes href
    return getattr(page, "_path", page.href)
...
def get_page_json_path(page: PageLike) -> Path | None:
    ...
    Returns:
        ...
        or if the path would conflict with site-wide index.json
    ...
    if output_path.name == "index.html":
        json_path = output_path.parent / "index.json"
        # Skip root index.json to avoid conflict with site-wide search index
        # The site-wide index.json is generated by SiteIndexGenerator
        if json_path.name == "index.json" and str(json_path.parent).endswith(
            ("public", "output", "_site", "dist", "build")
        ):
            # Check if this is the root output directory by looking at parent structure
            # A more reliable check: if the json_path parent has no subdirectory component
            # relative to what would be the output dir
            site = getattr(page, "_site", None)
            if site:
                output_dir = getattr(site, "output_dir", None)
                if output_dir and json_path.parent == output_dir:
                    return None  # Skip root index.json
        return json_path
