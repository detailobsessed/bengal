...
class OutputFormatsGenerator:
    ...
    def generate(self) -> None:
        ...
        if "json" in per_page:
            ...
            # Note: Legacy has_accumulated_json fallback removed - unified accumulator is now required
        ...
        if "index_json" in site_wide:
            ...
            search_config_raw = self.site.config.get("search", {})
            search_config = (
                search_config_raw if isinstance(search_config_raw, dict) else {}
            )
            lunr_config_raw = search_config.get("lunr", {})
            lunr_config = lunr_config_raw if isinstance(lunr_config_raw, dict) else {}
                ...
                if lunr_gen.is_available():
                    ...
                    for index_path in index_paths:
                        ...
                        if lunr_path:
                            ...
                            logger.debug(
                                "generated_prebuilt_lunr_index", path=str(lunr_path)
                            )
                else:
                    logger.debug(
