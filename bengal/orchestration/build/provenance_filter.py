...
def phase_incremental_filter_provenance(
    ...
    with orchestrator.logger.phase(
        "incremental_filtering_provenance", enabled=incremental
    ):
        ...
        if (
            incremental
            and result.pages_skipped
            and cache
            and hasattr(cache, "output_sources")
        ):
            skipped_by_source = {
                str(page.source_path): page for page in result.pages_skipped
            }
            missing_pages: list = []
            ...
            if missing_pages:
                ...
                pages_skipped = [
                    p for p in result.pages_skipped if p not in missing_pages
                ]
        ...
        if result.cache_hits > 0:
            ...
            orchestrator.stats.time_saved_ms = (
                result.cache_hits * avg_time_per_page * 0.8
            )
        ...
        cli.info(
            f"  Provenance build: {pages_msg}, {assets_msg} (skipped {skipped_msg})"
        )
        cli.detail(
            f"Filter time: {filter_time_ms:.1f}ms ({result.hit_rate:.1f}% hit rate)",
            indent=1,
        )

        # Store provenance filter for later use (recording builds)
        object.__setattr__(orchestrator, "_provenance_filter", provenance_filter)
...
def record_page_build(orchestrator: BuildOrchestrator, page) -> None:
    ...
    """
    pf = getattr(orchestrator, "_provenance_filter", None)
    if pf is not None:
