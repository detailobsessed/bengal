...
def phase_parse_content(
                    ...
                    try:
                        ...
                        if orchestrator.stats:
                            orchestrator.stats.add_error(
                                Exception(
                                    f"Parsing failed for {page.source_path}: {e}"
                                ),
                                category="parsing",
                            )
        else:
    ...
    # Track cache statistics (attribute may not exist on all stats implementations)
    # Use object.__setattr__ to avoid pyright error on dynamic attribute
    if hasattr(orchestrator.stats, "parsing_cache_hits"):
        object.__setattr__(orchestrator.stats, "parsing_cache_hits", cache_hits)
