...
class PageRankCalculator:
    ...
    def compute(
        ...
        if personalized and seed_pages:
            personalization = {
                page: (1.0 / len(seed_pages) if page in seed_pages else 0.0)
                for page in pages
            }
        else:
        ...
        for iteration in range(self.max_iterations):
            ...
            if max_diff < self.threshold:
                converged = True
                logger.info(
                    "pagerank_converged", iterations=iterations_run, max_diff=max_diff
                )
                break

        if not converged:
            logger.warning(
                "pagerank_max_iterations", iterations=iterations_run, max_diff=max_diff
            )
