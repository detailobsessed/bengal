...
class ReloadController:
    ...
    def _diff(
        self, prev: OutputSnapshot, curr: OutputSnapshot
        ...
        for path, entry in curr_files.items():
            ...
            if (
                pentry is None
                or pentry.size != entry.size
                or pentry.mtime != entry.mtime
            ):
                changed.append(path)
    ...
    def decide_and_update(self, output_dir: Path) -> ReloadDecision:
        ...
        if changed and self._hash_on_suspect:
            ...
            for path in changed:
                ...
                if (
                    is_suspect
                    and suspects_hashed < self._suspect_hash_limit
                    and centry is not None
                ):
                    try:
                        ...
                        digest = hash_file(
                            abs_path, algorithm="md5", chunk_size=1 << 16
                        )
                        suspects_hashed += 1
        ...
        if not changed:
            return ReloadDecision(
                action="none", reason="no-output-change", changed_paths=[]
            )
