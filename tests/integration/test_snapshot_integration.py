...
def test_snapshot_vs_sequential_rendering(site, build_site):
    ...
    parallel_html = {
        p.output_path: p.output_path.read_text()
        if p.output_path and p.output_path.exists()
        else ""
        for p in site.pages
        if p.output_path
    }
    ...
    assert set(parallel_html.keys()) == set(sequential_html.keys()), (
        "Different pages rendered"
    )
