...
class TestHighConcurrencyStress:
    ...
    def test_indexing_thread_safety_large_scale(self, tmp_path: Path):
        ...
        for m_idx in range(num_modules):
            ...
            module_content.extend(f"class {c_name}: pass" for c_name in colliding_names)
        ...
        with (
            ...
            patch(
                "bengal.autodoc.extractors.python.extractor.get_optimal_workers",
                return_value=16,
            ),
        ):
    ...
    def test_inheritance_resolution_in_stress_env(self, tmp_path: Path):
        ...
        extractor = PythonExtractor(
            config={"include_inherited": True, "parallel_autodoc": True}
        )
            ...
            if "derived" in element.name:
                for child in element.children:
                    if child.element_type == "class":
                        ...
                        # Look for method{i-1}
                        inherited_methods = [
                            m for m in method_names if m.startswith("method")
                        ]
