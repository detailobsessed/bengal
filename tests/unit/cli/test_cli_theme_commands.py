...
def _fake_theme(tmp_path: Path, slug: str = "acme"):
    ...
    (tmp_path / "bengal_themes" / "__init__.py").write_text(
...
def test_theme_list_and_info(tmp_path, monkeypatch):
    ...
    r2 = CliRunner().invoke(
        cli_main, ["utils", "theme", "info", "acme", str(site_root)]
    )
...
class TestThemeInstallSecurity:
    ...
    def test_rejects_shell_injection(self):
        ...
        r = CliRunner().invoke(
            cli_main, ["utils", "theme", "install", "theme; rm -rf /"]
        )
        assert r.exit_code != 0
        assert "Invalid package name" in r.output
    ...
    def test_rejects_dollar_injection(self):
        ...
        r = CliRunner().invoke(
            cli_main, ["utils", "theme", "install", "theme$(whoami)"]
        )
        assert r.exit_code != 0
        assert "Invalid package name" in r.output
    ...
    def test_rejects_pipe_injection(self):
        ...
        r = CliRunner().invoke(
            cli_main, ["utils", "theme", "install", "theme|cat /etc/passwd"]
        )
        assert r.exit_code != 0
        assert "Invalid package name" in r.output
    ...
    def test_accepts_valid_package_name(self):
        ...
        r = CliRunner().invoke(
            cli_main, ["utils", "theme", "install", "bengal-theme-test"]
        )
    ...
    def test_accepts_name_with_dots(self):
        ...
        r = CliRunner().invoke(
            cli_main, ["utils", "theme", "install", "bengal-theme-test.1.0"]
        )
        assert "Invalid package name" not in r.output
    ...
    def test_accepts_name_with_underscores(self):
        ...
        r = CliRunner().invoke(
            cli_main, ["utils", "theme", "install", "bengal_theme_test"]
        )
        assert "Invalid package name" not in r.output
