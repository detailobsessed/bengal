...
class TestGeneratedPageCache:
    ...
    def test_should_regenerate_true_when_template_changed(self, tmp_path: Path) -> None:
        ...
        cache.update(
            "tag", "python", pages, hashes, "<html>v1</html>", 100, "template_v1"
        )

        # Same content, different template
        assert (
            cache.should_regenerate("tag", "python", pages, hashes, "template_v1")
            is False
        )
        assert (
            cache.should_regenerate("tag", "python", pages, hashes, "template_v2")
            is True
        )
