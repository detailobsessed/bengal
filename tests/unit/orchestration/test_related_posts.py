...
def test_related_posts_with_shared_tags(mock_site):
    ...
    assert page2 in page1.related_posts, (
        "page2 should be related to page1 (shared tag: python)"
    )
    assert page1 in page2.related_posts, (
        "page1 should be related to page2 (shared tag: python)"
    )

    # page3 has no shared tags with page1 or page2
    assert page3 not in page1.related_posts, (
        "page3 should not be related to page1 (no shared tags)"
    )
    assert page1 not in page3.related_posts, (
        "page1 should not be related to page3 (no shared tags)"
    )
...
def test_related_posts_sorted_by_relevance(mock_site):
    ...
    page1 = Page(
        source_path=Path("page1.md"),
        metadata={"title": "Post 1", "tags": ["a", "b", "c"]},
    )
    page2 = Page(
        source_path=Path("page2.md"),
        metadata={"title": "Post 2", "tags": ["a", "b", "c"]},
    )  # 3 shared
    page3 = Page(
...
def test_related_posts_skips_generated_pages(mock_site):
    ...
    assert page2 not in page1.related_posts, (
        "Generated pages should be excluded from related posts"
    )
...
def test_related_posts_no_tags(mock_site):
    ...
    assert len(page1.related_posts) == 0, (
        "Pages without tags should have no related posts"
    )
