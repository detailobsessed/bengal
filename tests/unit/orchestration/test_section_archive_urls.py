...
class TestAutoGeneratedArchives:
    ...
    def test_auto_archive_created(self, site_without_index):
        ...
        assert blog_section.index_page.metadata.get("_generated"), (
            "Should be marked generated"
        )
    ...
    def test_auto_archive_child_urls(self, site_without_index):
        ...
        assert all(p.href.startswith("/blog/") for p in posts)
    ...
    def test_archive_metadata_posts_list(self, site_without_index):
        ...
        assert all(p.href.startswith("/blog/") for p in posts_list)
...
class TestArchiveWithCascade:
    ...
    def test_cascaded_archive_child_urls(self, cascaded_archive_site):
        ...
        guides = [
            p
            for p in cascaded_archive_site.pages
            if "guides/guide-" in str(p.source_path)
        ]

        assert len(guides) >= 2

        # All should have full path /docs/guides/
        assert all(g.href.startswith("/docs/guides/") for g in guides)
        # Type should be cascaded
        assert all(g.metadata.get("type") == "doc" for g in guides)
...
class TestNestedArchives:
    ...
    def test_deeply_nested_archive_urls(self, nested_archive_site):
        ...
        endpoints = [
            p
            for p in nested_archive_site.pages
            if p.source_path.stem in ("users", "posts")
        ]

        assert len(endpoints) >= 2

        assert all(e.href.startswith("/api/v2/endpoints/") for e in endpoints)
    ...
    def test_mixed_sections_all_urls_correct(self, mixed_archive_site):
        ...
        for page in all_pages:
            ...
            if "with-index" in path_str:
                ...
                assert page.href.startswith("/mixed/"), (
                    f"Root section page wrong URL: {page.href}"
                )
...
class TestArchiveURLConsistency:
    ...
    def test_archive_and_manual_index_same_behavior(self):
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            # Find pages
            page_a = next(
                p for p in site.pages if "section-a/page" in str(p.source_path)
            )
            page_b = next(
                p for p in site.pages if "section-b/page" in str(p.source_path)
            )

            # Both should follow same URL pattern
            assert page_a.href == "/section-a/page/", (
                f"Manual index page wrong: {page_a.href}"
            )
            assert page_b.href == "/section-b/page/", (
                f"Auto-archive page wrong: {page_b.href}"
            )
